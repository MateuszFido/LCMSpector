...
def integrate_ms_xic_peak(
    ...
    try:
        start_idx, end_idx, baseline_start, baseline_end = detect_peak_boundaries(
            times_sorted, intensities_sorted, peak_index, rt_target,
            min_peak_width, max_peak_width, noise_threshold
        )
...
def integrate_lc_peak(
    ...
    try:
        start_idx, end_idx, baseline_start, baseline_end = detect_peak_boundaries(
            retention_times, baseline_corrected, peak_index, rt_target,
            min_peak_width, max_peak_width, noise_threshold
        )
    except BoundaryDetectionError:
        start_idx, end_idx, baseline_start, baseline_end = fallback_boundary_detection(
            retention_times, baseline_corrected, peak_index, min_peak_width, max_peak_width
        )
    ...
    snr, quality_score = calculate_peak_quality_metrics(
        retention_times, baseline_corrected, dummy_baseline, start_idx, end_idx, peak_index
    )
    ...
    return {
        ...
        "start_index": int(start_idx),
        "end_index": int(end_idx),
        "baseline_start": 0.0,  # Already baseline corrected
        "baseline_end": 0.0,    # Already baseline corrected
        "peak_height": float(peak_height),
        "integration_method": "trapezoidal",
        "snr": float(snr),
        "quality_score": float(quality_score)
...
def detect_peak_boundaries(
    ...
    signal_baseline = np.percentile(intensities, 10)  # 10th percentile as baseline estimate
    ...
    for i in range(peak_index + 1, len(times)):
        ...
        # Check for valley (local minimum)
        if i < len(times) - 1 and intensities[i] < intensities[i-1] and intensities[i] < intensities[i+1]:
            if intensities[i] <= min_intensity * 2:
                end_idx = i
                break
...
def calculate_peak_quality_metrics(
    ...
    baseline_at_peak = baseline[peak_index - start_index] if len(baseline) > peak_index - start_index else 0
    ...
    if len(noise_regions) > 0:
        ...
        if len(all_intensities) > 10:
            noise = np.std(all_intensities[all_intensities <= np.percentile(all_intensities, 20)])
    ...
    snr_score = min(1.0, snr / 50.0)  # Better normalization for LC-MS (SNR can be >1000)
    ...
    if len(peak_region) > 4:
        ...
        left_10pct_idx = np.argmin(np.abs(peak_region[:peak_index - start_index] - height_10pct)) if peak_index > start_index else 0
        right_10pct_idx = peak_index - start_index + np.argmin(np.abs(peak_region[peak_index - start_index:] - height_10pct))
        
        if right_10pct_idx > left_10pct_idx:
            tailing_factor = (right_10pct_idx - left_10pct_idx) / (2 * (peak_index - start_index - left_10pct_idx))
            tailing_score = max(0.0, 1.0 - abs(tailing_factor - 1.0))  # Ideal tailing factor is 1.0
...
def thread_safe_peak_integration(compounds_data, mass_accuracy, file_name):
    ...
    for compound in safe_compounds:
        try:
            processed_compound = process_compound_peak_areas(compound, mass_accuracy, file_name)
...
def process_compound_peak_areas(compound, mass_accuracy, file_name):
    ...
    for ion in compound.ions.keys():
        try:
            ...
            logger.warning(f"Peak area calculation failed for ion {ion} in {compound.name}: {e}")
            compound.ions[ion]['MS Peak Area'] = create_fallback_peak_area()
