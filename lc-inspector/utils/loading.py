...
def load_absorbance_data(file_path):
    ...
    with open(file_path, "r", newline=None) as file:
        ...
        for i in range(5):
            ...
            if delimiter is not None and detect_delimiter(line) != delimiter:
                logger.error(
                    "Detected more than 1 different delimiters in the file. Double check for possible parsing errors."
                )
    ...
    chromatogram_data = pd.DataFrame(
        {"Time (min)": time_values, "Value (mAU)": intensity_values}
    )
...
def load_annotated_peaks(file_path):
    ...
    peakname = (
        r"(Peakname|Name)\s*"  # Matches "Peakname" or "Name" with optional whitespace
    )
    ...
    peak_stop = (
        r"(Peak Stop|Stop)\s*"  # Matches "Peak Stop" or "Stop" with optional whitespace
    )
    ...
    df = pd.read_csv(
        ...
        delimiter="\t",
        header=0,
        usecols=lambda x: re.search(peakname, x)
        or re.search(rt, x)
        or re.search(area, x)
        or re.search(peak_start, x)
        or re.search(peak_stop, x),
    )
